// src/data/co-founders.js
import panagiotisImg from "../assets/panagiotis.jpg";
import iliasImg from "../assets/ilias.jpg";
import antonisImg from "../assets/antonis.jpg";
import giorgosImg from "../assets/giorgos.jpg";
import vaggelisImg from "../assets/vaggelis.png";
import foteiniImg from "../assets/foteini.png";
import sotirisImg from "../assets/sotiris.png";
import mariaImg from "../assets/maria.png";

export const cofounders = [
  {
    name: "Panagiotis Kotsalis",
    image: panagiotisImg,
    bio: `Panagiotis has experience in active citizenship and leadership through youth councils. He promotes civic engagement and intercultural dialogue, inspiring youth with values of unity and cooperation.`,
    bio_el: `Ο Παναγιώτης έχει εμπειρία στην ενεργό συμμετοχή των νέων και την ηγεσία μέσα από συμβούλια νέων. Ενθαρρύνει την πολιτική δράση και τον διάλογο μεταξύ διαφορετικών πολιτισμών, εμπνέοντας τους νέους με αξίες ενότητας και συνεργασίας.`,
  },
  {
    name: "Ilias Kalantzis",
    image: iliasImg,
    bio: `Ilias is a Business Management graduate pursuing a master's in finance. He has led youth camps, participated in Erasmus+ projects, and received awards for entrepreneurship and presentations.`,
    bio_el: `Ο Ηλίας είναι απόφοιτος Διοίκησης Επιχειρήσεων και συνεχίζει τις σπουδές του στα οικονομικά. Έχει οργανώσει κατασκηνώσεις νέων, συμμετάσχει σε προγράμματα Erasmus+ και έχει βραβευτεί για την επιχειρηματικότητα και τις παρουσιάσεις του.`,
  },
  {
    name: "Antonis Kalantzis",
    image: antonisImg,
    bio: `Antonis is a technology enthusiast with interests in cloud computing, AI agents, and emerging innovations. As co-founder of Odyssey of Wisdom, he works to inspire youth through technology and collaboration.`,
    bio_el: `Ο Αντώνης ασχολείται με την τεχνολογία και ενδιαφέρεται για το cloud computing, την τεχνητή νοημοσύνη και τις νέες καινοτομίες. Ως συνιδρυτής του Odyssey of Wisdom, θέλει να εμπνεύσει νέους ανθρώπους μέσα από την τεχνολογία και τη συνεργασία.`,
  },
  {
    name: "Giorgos Tsitlakidis",
    image: giorgosImg,
    bio: `Giorgos holds a Mathematics degree and studies Computer Science. After discovering Erasmus+, he developed a passion for youth exchanges and now empowers others as a math teacher and project leader.`,
    bio_el: `Ο Γιώργος έχει πτυχίο Μαθηματικών και σπουδάζει Πληροφορική. Μέσα από το Erasmus+ ανακάλυψε το ενδιαφέρον του για τις ανταλλαγές νέων και σήμερα ενθαρρύνει άλλους μέσα από τη δουλειά του ως καθηγητής μαθηματικών και υπεύθυνος έργων.`,
  },
  {
    name: "Vaggelis Argyropoulos",
    image: vaggelisImg,
    bio: `Vaggelis is a computer scientist interested in how technology affects everyday life. He values learning through travel and cross-cultural exchange, and enjoys being part of different places and communities.`,
    bio_el: `Ο Βαγγέλης έχει σπουδάσει Πληροφορική και τον ενδιαφέρει πώς η τεχνολογία επηρεάζει την καθημερινή ζωή. Του αρέσει να μαθαίνει μέσα από ταξίδια και γνωριμίες με διαφορετικούς πολιτισμούς, και απολαμβάνει να είναι κομμάτι διαφόρων κοινοτήτων.`,
  },
  {
    name: "Foteini Karamitsou",
    image: foteiniImg,
    bio: `Foteini is a special education student, passionate about youth work and storytelling. Curious about cultures, she shares Erasmus+ stories through blogging and social media.`,
    bio_el: `Η Φωτεινή σπουδάζει Ειδική Αγωγή και της αρέσει να δουλεύει με νέους και να αφηγείται ιστορίες. Έχει μεγάλη περιέργεια για τους πολιτισμούς και μοιράζεται εμπειρίες από το Erasmus+ μέσα από το blogging και τα social media.`,
  },
  {
    name: "Maria Gagani",
    image: mariaImg,
    bio: `Maria is a Business Administration student and volunteer with a passion for learning. Through Erasmus+ and student organizations, she seeks travel and cultural experiences for her personal fulfillment.`,
    bio_el: `Η Μαρία σπουδάζει Διοίκηση Επιχειρήσεων και είναι εθελόντρια με πάθος για τη μάθηση. Μέσα από το Erasmus+ και τις φοιτητικές οργανώσεις, αναζητά ταξίδια και πολιτισμικές εμπειρίες για την προσωπική της ολοκλήρωση.`,
  },
  {
    name: "Sotiris Kouroukafopoulos",
    image: sotirisImg,
    bio: `Sotirios is a 21-year-old Law student from Greece, active in Erasmus+ and volunteering. He's a youth leader with strong teamwork skills and a passion for intercultural learning.`,
    bio_el: `Ο Σωτήρης είναι 21 ετών και φοιτητής Νομικής από την Ελλάδα. Είναι ενεργός στο Erasmus+ και τον εθελοντισμό, με πάθος για τη διαπολιτισμική μάθηση και την ομαδική συνεργασία.`,
  },
];
